...
SemaphoreHandle_t xSerial1SemaphoreSensor1;
SemaphoreHandle_t xSerial1SemaphoreSensor2;
SemaphoreHandle_t xSerial1SemaphoreSensor3;
SemaphoreHandle_t xSerial1SemaphorePower;
...
void setup() {
   ...
   // configure Arduino LED pin for output
    ...
    if ( xSerial1SemaphoreSensor1 == NULL )  // Check to confirm that the Serial1 Semaphore has not already been created.
    {
      xSerial1SemaphoreSensor1 = xSemaphoreCreateBinary(); 
      if ( ( xSerial1SemaphoreSensor1 ) != NULL ) xSemaphoreGive( ( xSerial1SemaphoreSensor1 ) ); 
    }
    if ( xSerial1SemaphorePower == NULL )  // Check to confirm that the Serial1 Semaphore has not already been created.
    {
      xSerial1SemaphorePower = xSemaphoreCreateBinary(); 
      if ( ( xSerial1SemaphorePower ) != NULL ) xSemaphoreGive( ( xSerial1SemaphorePower ) ); 
    }

    if ( xBufferMutex == NULL )  // Check to confirm that the Serial1 Semaphore has not already been created.
    {
...

{
 ...
 // const TickType_t xPeriod = pdMS_TO_TICKS( 32 );
  ...
  for (;;)
  {
    ...
    if ( xSemaphoreTake( xSerial1SemaphoreSensor1, portMAX_DELAY) ) {
...
//        dtostrf(ax2_f, 5, 1, ax2_c);
//        dtostrf(ay2_f, 5, 1, ay2_c);
//        dtostrf(az2_f, 5, 1, az2_c);
//        dtostrf(gx2_f, 5, 1, gx2_c);
//        dtostrf(gy2_f, 5, 1, gy2_c);
//        dtostrf(gz2_f, 5, 1, gz2_c);
...
//        dtostrf(ax3_f, 5, 1, ax3_c);
//        dtostrf(ay3_f, 5, 1, ay3_c);
//        dtostrf(az3_f, 5, 1, az3_c);
//        dtostrf(gx3_f, 5, 1, gx3_c);
//        dtostrf(gy3_f, 5, 1, gy3_c);
//        dtostrf(gz3_f, 5, 1, gz3_c);
...
void TaskReadPower(void *pvParameters)  // This is a task.
{
  ...
  for (;;)
  {
    ...
    if ( xSemaphoreTake( xSerial1SemaphorePower, portMAX_DELAY) )
    {
      if ( xSemaphoreTake( xBufferMutex, portMAX_DELAY )) {
...
//      
     ...
     // if no ack, add to buffer
      ...
      xSemaphoreGive( xBufferMutex );
      xSemaphoreGive( xSerial1SemaphoreSensor1 );
      xSemaphoreGive( xSerial1SemaphorePower );
