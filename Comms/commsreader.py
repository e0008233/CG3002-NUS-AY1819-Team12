...
def Msg_Restart():
    restart_flag = input('Do you want to reset message id? y if you want to, press any other key if you do not: ')
    if restart_flag == 'y':
        print('Restarting')
        ser.write(ackRestart)
    else: 
        print('Starting')
        ser.write(ack)
...
def finalPrediction(prediction):
    predict_dict = {'wipers':0, 'number7':0, 'chicken':0, 'sidestep':0, 'turnclap':0, 'cowboy':0, 'salute':0, 'number6':0, 'swing':0, 'mermaid':0}
    weights = [1,1,2,2,3]
...
def calculatePower(I, V):
    ...
    cum_power += power * float((current_milli_time() - power_millis - 500)/3600)
    power_millis = current_milli_time()
    return power
...
ack = ('A').encode('utf-8')
nack = ('N').encode('utf-8')
ackRestart = ('K').encode('utf-8')
samples = []
startIndex = 0
endIndex = 72
incrementAmt = float(endIndex * 0.50)
currentIndex = 0
prediction_mlp = []
prediction_rf = []
cum_power = 0.0
...
time_previous = current_milli_time()
power_millis = current_milli_time()
prediction_count = 0
dance_count = 0
ser.flush()
    ...
    if message == 'A' and message != '':
        ...
        if isPrintable(msg_full):
            ...
            if str(transmitted_checksum).strip() == str(msg_checksum).strip():
                ...
                ##### Print Frequency every 100 data received ####
                ### Used in training mode ###
                count +=1
                if count >=100:
                    time_now = current_milli_time()
                    count = 0
                    frequency = 1.0/((float(time_now - time_previous))/100000)
                    time_previous = current_milli_time()
        ...
        if match_flag == True:
            ...
            ##### Prints message every 200 received #####
            if message_id.isdigit():
                if (int(message_id) % 200 == 0):
                    print("Message: %s" % msg_full)
            ...
            ##### Live mode #####
                ...
                ##### Runs prediction when there are enough data #####
                if len(samples) >= endIndex:
                    # Check frequency
                    freq2_millis = current_milli_time()
                    freq = float(incrementAmt) / float((freq2_millis - freq1_millis)/1000)
                    print('Frequency: ' + str(freq))
                    freq1_millis = current_milli_time()
                    mlp_result = prediction.predict(samples[int(startIndex):int(endIndex)])
                    ##### Shifts the indexes for the next iteration #####
                    startIndex += incrementAmt
                    endIndex += incrementAmt

                    prediction_mlp.append(mlp_result)
                    if len(prediction_mlp) >= 3:
                        ##### Checks the last 2 prediction and there are less than 10 extra predictions #####
                        if prediction_mlp[-1] == prediction_mlp[-2] or prediction_count >= 10:
                            predicted_move = prediction_mlp[-1]

                            ##### Compute power here #####
                            power = calculatePower(voltage,current)

                            if predicted_move == 'byebye' and dance_count >= 40:
                                predicted_move = 'logout'
                            if predicted_move == 'number6':
                                predicted_move = 'numbersix'
                            server_message = '#' + predicted_move + '|' + str(voltage) + '|' + str(current) + '|' + str(power) + '|' + str(cum_power)
                            
                            if debug==0:
                                client.send(encryptData(server_message))

                            ##### Shifts the indexes again to avoid overlapping in the next dance move #####
                            startIndex += incrementAmt
                            endIndex += incrementAmt

                            ##### Keeps track of number of dances to avoid sending logout to server preemptively#####
                            dance_count += 1
                            print('\n=========================\n Final prediction: ' + predicted_move + '\n=========================\n')
                            
                            ##### Clears buffer and goes to sleep #####
                            prediction_mlp = []
                            prediction_count = 0
                            time.sleep(0.5)

                        ##### Else continue getting next prediction #####
                        else:
                            print('Confused! getting one more prediction...')
                            prediction_count += 1

    else:
        ##### Goes into rehandshake mode if there is no legitimate data received from Arduino #####
